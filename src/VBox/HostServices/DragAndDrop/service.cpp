...
{ \
    uint32_t cbTemp = 0; \
    rc = a_ParmUInt32.getUInt32(&cbTemp); \
                ...
                switch (pClient->protocol())
                {
                    ...
                    default:
                    {
                        if (cParms == 5)
                        {
                            ...
                            RT_ZERO(data);
                            data.hdr.uMagic = CB_MAGIC_DND_GH_SND_FILE_DATA;
                            rc = paParms[0].getPointer((void**)&data.u.v1.pszFilePath, &data.u.v1.cbFilePath);
                            VERIFY_BUFFER_SIZE_UINT32(paParms[1], data.u.v1.cbFilePath);
                            if (RT_SUCCESS(rc))
                                rc = paParms[2].getPointer((void**)&data.pvData, &data.cbData);
