...
void UIMediumManagerWidget::sltApplyMediumDetailsChanges()
{
    ...
    // WORKAROUND:
    // By default we need to update certain item only but
    // in rare cases when there are no other events at all
    // we need to start medium enumeration and be gone with it ..
    bool fStartMediumEnumeration = false;
        ...
        && newData.m_options.m_strLocation != oldData.m_options.m_strLocation)
    {
        ...
        else
        {
            ...
            {
                // WORKAROUND:
                // Since in 6.0 we don't have medium related events at all yet and no
                // machine/snapshot related events in that case as well we will use
                // that no so cheap decision, in 6.1 it is already working properly..
                fStartMediumEnumeration = true;
            }
        }
    }
    ...
    /* Start full medium enumeration: */
    if (fStartMediumEnumeration)
