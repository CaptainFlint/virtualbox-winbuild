...
void UIMachineWindowFullscreen::showInNecessaryMode()
{
    ...
    else
    {
        ...
        /* For some reason window resize events are not handled properly for minimized machine-windows.
         * So showing in full-screen first and then minimizing it: */
        const bool fIsMinimized = isMinimized();

        /* Show window in fullscreen mode first: */
        showFullScreen();

        /* If window was minimized, show window in minimized mode: */
        if (fIsMinimized)
            showMinimized();
#endif /* Q_WS_WIN || Q_WS_X11 */
