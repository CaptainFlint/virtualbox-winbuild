...
static int buslogicRegisterRead(PBUSLOGIC pBusLogic, unsigned iRegister, uint32_t *pu32)
{
    ...
    switch (iRegister)
    {
        ...
        {
            AssertCompileSize(pBusLogic->LocalRam, 256);
            AssertCompileSize(pBusLogic->iReply, sizeof(uint8_t));
            AssertCompileSize(pBusLogic->cbReplyParametersLeft, sizeof(uint8_t));
            ...
            else
            {
                 ...
                 */
                if (pBusLogic->iReply >= sizeof(pBusLogic->aReplyBuffer))
                    *pu32 = 0;
                else
