...
static void drvHostCoreAudioPCMPropsToASBD(PDMPCMPROPS *pPCMProps, AudioStreamBasicDescription *pASBD)
{
 ...
 */
static int drvHostCoreAudioInitConvCbCtx(PCOREAUDIOCONVCBCTX pConvCbCtx,
 ...
 */
static void drvHostCoreAudioUninitConvCbCtx(PCOREAUDIOCONVCBCTX pConvCbCtx)
{
                                                                                  ...
                                                                                  void                             *pvUser)
{
    ...
    switch (propertyID)
    {
        ...
        {
            LogRel(("CoreAudio: Capturing sample rate changed\n"));
...
static DECLCALLBACK(OSStatus) drvHostCoreAudioConverterCallback(AudioConverterRef              inAudioConverter,
                                                                ...
                                                                void                          *pvUser)
{
    ...
    if (*ioNumberDataPackets)
    {
        ...
        LogFlowFunc(("cbOff=%zu, cbAvail=%zu\n", cbOff, cbAvail));
    ...
    LogFlowFunc(("%RU32 / %RU32 -> ioNumberDataPackets=%RU32\n",
                                                                ...
                                                                AudioBufferList            *pBufData)
{
    ...
    LogFlowFunc(("cFrames=%RU32 (%RU32 frames per packet) -> %RU32 packets\n",
    ...
    do
    {
        ...
        if (pStreamIn->pConverter)
        {
            ...
            LogFlowFunc(("cPacketsToWrite=%RU32\n", cPacketsToWriteAndWritten));

            do
            {
                err = AudioConverterFillComplexBuffer(pStreamIn->pConverter,
                                                      drvHostCoreAudioConverterCallback, pConvCbCtx /* pvData */,
                                                      &cPacketsToWriteAndWritten, &dstBufList, NULL);

                LogFlowFunc(("cPacketsWritten=%RU32 (%zu bytes), err=%RI32\n",
...
static int drvHostCoreAudioInitInput(PPDMIHOSTAUDIO pInterface, PPDMAUDIOHSTSTRMIN pHstStrmIn, uint32_t *pcSamples)
{
    ...
    do
    {
        ...
        drvHostCoreAudioPCMPropsToASBD(&pStreamIn->streamIn.Props, &pStreamIn->streamFormat);

        drvHostCoreAudioPrintASBD("Recording device", &pStreamIn->deviceFormat);
        drvHostCoreAudioPrintASBD("Recording stream", &pStreamIn->streamFormat);
            ...
            || pStreamIn->deviceFormat.mChannelsPerFrame != pStreamIn->streamFormat.mChannelsPerFrame)
        {
            ...
            err = AudioConverterNew(&pStreamIn->deviceFormat /* Input */, &pStreamIn->streamFormat /* Output */,
                                    &pStreamIn->pConverter);
            if (RT_UNLIKELY(err != noErr))
            {
        ...
        if (RT_SUCCESS(rc))
            rc = RTCircBufCreate(&pStreamIn->pCircBuf, cSamples << pHstStrmIn->Props.cShift);

        /* Init the converter callback context. */
        if (RT_SUCCESS(rc))
            rc = drvHostCoreAudioInitConvCbCtx(&pStreamIn->convCbCtx,
...
static int drvHostCoreAudioInitOutput(PPDMIHOSTAUDIO pInterface, PPDMAUDIOHSTSTRMOUT pHstStrmOut, uint32_t *pcSamples)
{
    ...
    do
    {
        ...
        /* Create an AudioStreamBasicDescription based on our required audio settings. */
        drvHostCoreAudioPCMPropsToASBD(&pStreamOut->streamOut.Props, &pStreamOut->streamFormat);

        drvHostCoreAudioPrintASBD("Playback device", &pStreamOut->deviceFormat);
        drvHostCoreAudioPrintASBD("Output format", &pStreamOut->streamFormat);
        ...
        rc = RTCircBufCreate(&pStreamOut->pCircBuf, cSamples << pHstStrmOut->Props.cShift);
        if (RT_SUCCESS(rc))
        {
            /*
             * Register callbacks.
                                                   ...
                                                   uint32_t *pcSamplesCaptured)
{
    ...
    do
    {
        size_t cbBuf     = AudioMixBufFreeBytes(&pHstStrmIn->MixBuf);
        size_t cbToWrite = RTCircBufUsed(pStreamIn->pCircBuf);

        LogFlowFunc(("cbBuf=%zu, cbToWrite=%zu/%zu\n", cbBuf, cbToWrite, RTCircBufSize(pStreamIn->pCircBuf)));
        ...
        while (cbToWrite)
        {
            ...
            if (RT_SUCCESS(rc))
            {
                ...
                AssertFailed();
#endif
            rc = AudioMixBufWriteCirc(&pHstStrmIn->MixBuf, puBuf + cbWrittenTotal, cbToRead, &cWritten);
    ...
    if (RT_SUCCESS(rc))
    {
        if (cWrittenTotal)
        {
            ...
            LogFlowFunc(("cWrittenTotal=%RU32 (%RU32 bytes), cCaptured=%RU32, rc=%Rrc\n",
                         cWrittenTotal, AUDIOMIXBUF_S2B(&pHstStrmIn->MixBuf, cWrittenTotal), cCaptured, rc));
            }
    }
                                                 ...
                                                 uint32_t *pcSamplesPlayed)
{
    ...
    size_t cbToRead = RT_MIN(cbAvail, RTCircBufFree(pStreamOut->pCircBuf));
