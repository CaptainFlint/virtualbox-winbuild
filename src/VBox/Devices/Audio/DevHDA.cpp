...
static void      hdaStreamTransfer(PHDASTATE pThis, PHDASTREAM pStream);
...
static int hdaStreamInit(PHDASTATE pThis, PHDASTREAM pStream, uint8_t uSD)
{
    ...
    const uint16_t u16FIFOS = HDA_STREAM_REG(pThis, FIFOS, pStream->u8SD);
    Assert(u16FIFOS);
    pStream->u16FIFOS       = hdaSDFIFOSToBytes(u16FIFOS);
        ...
        && pStream->u16LVI)
    {
        ...
        if (pStream->State.strmCfg.uHz % pThis->u16TimerHz != 0)
            LogRel(("HDA: Device timer (%RU32) does not fit to stream #RU8 timing (%RU32)\n",
                    pThis->u16TimerHz, pStream->State.strmCfg.uHz));
        ...
        pStream->State.cbTransferChunk = (pStream->State.strmCfg.uHz / pThis->u16TimerHz) * HDA_FRAME_SIZE;
        ...
        pStream->State.tsTransferLast     = TMTimerGet(pThis->pTimer);
        pStream->State.tsTransferNext     = pStream->State.tsTransferLast + pStream->State.cTransferTicks;
...
static int hdaWriteAudio(PHDASTATE pThis, PHDASTREAM pStream)
{
    ...
    while (cbToWrite)
    {
        Assert(cbToWrite % HDA_FRAME_SIZE == 0);
...
static DECLCALLBACK(void) hdaTimer(PPDMDEVINS pDevIns, PTMTIMER pTimer, void *pvUser)
{
    ...
    if (cbOutMin)
    {
        ...
        hdaStreamTransfer(pThis, &pThis->Out);
    ...
    if (RT_SUCCESS(rc2))
        hdaStreamTransfer(pThis, &pThis->LineIn);

    /* If no streams are active anymore but there still is output data to process on the host side,
     * schedule another round until we're done. */
    if (   !pThis->cStreamsActive
        && csLiveMax)
    {
        Log3Func(("No active streams anymore, handling pending disable host output streams ...\n"));
...
static void hdaStreamTransfer(PHDASTATE pThis, PHDASTREAM pStream)
{
    ...
    /* Sanity checks. */
    Assert(pStream->u8SD <= HDA_MAX_STREAMS - 1);
