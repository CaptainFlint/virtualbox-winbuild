...
void mmR3PagePoolTerm(PVM pVM)
{
    if (pVM->mm.s.pPagePoolR3)
    {
        ...
        while (pSubPool)
        {
            int rc = SUPR3PageFreeEx(pSubPool->pvPagesR3, pSubPool->cPages);
            AssertMsgRC(rc, ("SUPR3PageFreeEx(%p) failed with rc=%Rrc\n", pSubPool->pvPagesR3, rc));
            pSubPool->pvPagesR3 = NULL;
            pSubPool->pvPagesR0 = NIL_RTR0PTR;
    ...
    if (pVM->mm.s.pPagePoolLowR3)
    {
        ...
        while (pSubPool)
        {
            int rc = SUPR3LowFree(pSubPool->pvPagesR3, pSubPool->cPages);
...
DECLINLINE(void *) mmR3PagePoolAlloc(PMMPAGEPOOL pPool)
{
    ...
    if (pPool->pHeadFree)
    {
        ...
        if (iPage >= 0)
        {
            ...
            return (uint8_t *)pSub->pvPagesR3 + PAGE_SIZE * iPage;
        }
#else
    ...
    if (!pPool->fLow)
    {
        ...
        rc = SUPR3LowAlloc(cPages, &pSub->pvPagesR3, &pSub->pvPagesR0, paPhysPages);
    if (RT_SUCCESS(rc))
    {
        /*
...
DECLINLINE(void) mmR3PagePoolFree(PMMPAGEPOOL pPool, void *pv)
{
    ...
    if (    !pLookup
        ||  (uint8_t *)pv >= (uint8_t *)pLookup->pSubPool->pvPagesR3 + (pLookup->pSubPool->cPages << PAGE_SHIFT)
        )
    {
        STAM_COUNTER_INC(&pPool->cErrors);
    ...
    /* clear bitmap bit */
    const unsigned  iPage = ((uint8_t *)pv - (uint8_t *)pSubPool->pvPagesR3) >> PAGE_SHIFT;
#ifdef USE_INLINE_ASM_BIT_OPS
