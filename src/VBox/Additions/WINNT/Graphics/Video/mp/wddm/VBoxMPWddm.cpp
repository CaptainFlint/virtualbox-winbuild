...
static NTSTATUS vboxWddmSetupDisplaysNew(PVBOXMP_DEVEXT pDevExt)
{
    ...
    /* The Gallium 3D driver (pDevExt->enmHwType == VBOXVIDEO_HWTYPE_VMSVGA) does not actually use CmdVbva.
     * But we still have to create the pDevExt->CmdVbva structure because some code elsewhere in the driver
     * depends on its fields. For example vboxWddmVramCpuVisibleSize.
     *
     * Also the Gallium 3D driver still needs 'pDevExt->fCmdVbvaEnabled = true' to enable relevant code paths.
     */
...
static int vboxWddmFreeDisplays(PVBOXMP_DEVEXT pDevExt)
{
    ...
    if (pDevExt->fCmdVbvaEnabled)
    {
        if (pDevExt->enmHwType == VBOXVIDEO_HWTYPE_VBOX)
        {
            ...
            if (RT_FAILURE(rc))
                WARN(("VBoxCmdVbvaDisable failed %d", rc));
        }

        if (RT_SUCCESS(rc))
        {
            /* Still required for VBOXVIDEO_HWTYPE_VMSVGA, see comment in vboxWddmSetupDisplaysNew. */
