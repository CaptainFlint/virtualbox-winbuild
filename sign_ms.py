...
class MsDashboardAPI:
	...
	def accessToken(self):
		...
		if not self._accessToken:
			raise Exception('Fatal error, exiting')
...
for ms_sign_task in attestation_sign_tasks:
	...
	product = ms_dashboard_client.createProduct(ms_sign_task['name'], ms_sign_task['osver'])
	if product is None:
	...
	submission = ms_dashboard_client.createSubmission(product['id'], ms_sign_task['name'])
	...
	if not sas_url:
		...
		sys.exit(1)
	if ms_dashboard_client.uploadFile(sas_url, ms_sign_task['src_file']) is None:
		output('Fatal error! Failed to upload archive; exiting.')
		sys.exit(1)
	output('Archive uploaded successfully.')

	commit = ms_dashboard_client.commitSubmission(product['id'], submission['id'])
	if commit is None:
...
while not finished:
	...
	for ms_sign_task in attestation_sign_tasks:
		...
		status = ms_dashboard_client.getSubmission(ms_sign_task['product_id'], ms_sign_task['submission_id'])
		if status is None:
