...
# Lock file and its handle
lockfile = re.sub(r'\.py.$', '.py', __file__) + '.lock'
lockfh = None
...
# If debug option is on, prints a message into stdout ensuring it will appear immediately
# Input:
...
def sign(basic_params, filepath):
    ...
    for sp in sign_params:
        if 'tsurl' not in sp or not sp['tsurl']:
            if sp['hash'] == 'sha1':
                sp['tsurl'] = tsURL_sha1
            else:
                sp['tsurl'] = tsURL_sha2
...
if __name__ == '__main__':
    ...
    sign1.add_argument('--tsurl1', action = 'store', required = False,
                       help = "Timestamping server URL, or 'none' for no timestamping; default: Digicert")
